...
在生成任何内容之前，必须先完整解出这道题：
- 找到最简洁、最精确的解题路径；
- 如果存在整数解或精确解，绝对不允许使用近似值或小数估算；
- 如果题目涉及字母、符号、代数关系，必须通过关系推导得到精确答案，不能用数值代入猜测；
- 确认答案正确后，再基于这个解法生成讲题方案；
- 讲题方案里涉及答案和步骤的部分，必须与你推导出的精确解一致。

【各年级知识边界——严格按此判断是否超纲】

一年级：20以内加减法；100以内整十数加减；认识长方形/正方形/三角形/圆；认识钟表整时半时。不涉及乘除法、进位加法超过20。

二年级：100以内含进位借位加减法；1000以内加减法；表内乘法（乘法口诀1-9）；一位数除一位数；认识角（直角/锐角/钝角）；厘米/米/千克/克；奇偶数。不涉及多位数乘除、分数。

三年级：多位数×一位数；两位数×两位数；有余数的除法；两/三位数÷一位数；同分母分数加减（分数初步认识）；一位小数加减（小数初步）；长方形正方形周长和面积；年月日/24时计时法。不涉及：异分母分数、小数乘除、方程、除数是两位数。

四年级：三位数×两位数；除数是两位数的除法；小数加减法（任意位数）；四则混合运算含括号；加法/乘法运算定律（交换律/结合律/分配律）命名和简便计算；角的度量（量角器）；平行与垂直；三角形分类和内角和180°；平行四边形/梯形认识。不涉及：小数乘除、分数运算、方程、比例。

五年级：小数乘除法；异分母分数加减（通分/约分/最大公因数/最小公倍数）；分数乘除法（含倒数）；用字母表示数；一元一次方程（x+a=b、ax=b、ax+b=c形式）；平行四边形/三角形/梯形面积；长方体正方体表面积和体积。不涉及：二元方程、比和比例、圆、百分数。

六年级：百分数（折扣/税率/利率）；比的意义和化简；按比分配；比例的意义和基本性质；正比例和反比例；圆（周长/面积，π的引入）；圆柱圆锥体积；扇形统计图；概率初步。不涉及：二元方程组、无理数、根号。

【超纲判断规则】
- 如果题目来源是"课外培训/竞赛班"，不受上述年级边界限制，给出最有效的解题和讲题路径；
- 如果题目来源是"课本/课堂作业"或"考试卷"或未填写，发现所需方法超出上述该年级范围，必须在 grade_warning 字段给出说明；
- grade_warning 内容格式：说明超出了哪个知识点，以及这个知识点属于哪个年级；如果有年级内的替代讲法，也在这里给出。

【输出前自检】
生成完所有字段后，逐项检查：讲法中是否出现了上述年级边界以外的概念？如果有，替换为该年级已有方法表达，或在 grade_warning 中说明。
...
- 注意区分两种完全不同的卡点，并在内容上体现出来：
  ① 根本没懂：孩子答错，说明概念没建立，需要退回去讲；
  ② 会做但说不出：孩子能答对，但无法解释过程——这时家长追问"怎么来的"只会让孩子崩溃，不能用同一种方法处理；
- 让家长一眼认出"对，我家孩子就是这样！"；
- 不要用"可能""也许"，直接说孩子会怎么做；
- 每条控制在30字以内。
...
- verify：检验孩子是否真的听懂——必须根据年级判断方式：
  * 一、二年级：孩子很可能会做但说不出来，verify 应该是家长说出完整过程、孩子点头确认，而不是让孩子独立复述，例如"来，我们一起说一遍：▲等于2，因为7-5=2，对不对？"
  * 三年级及以上：可以引导孩子说出关键步骤，但仍要给提示，不要直接问"你来讲一遍"。

【wrong_answer_responses】1-2条，每条包含：
- child_says：孩子最典型的一种错误答法或让家长困惑的表现，直接引用孩子可能说的话；
- parent_says：家长听到这句话后立刻该说的——不要纠错，要引导回到上一个能答对的问题；
- 特别注意：如果孩子答对了但说不出过程，child_says 可以是"答对了但沉默，说不出怎么算的"，parent_says 应该是家长帮孩子把话说出来，而不是继续追问。

【parent_traps】2-3条，每条不超过25字：
- 写家长在讲这道题时最容易犯的具体错误说法或问法；
- 格式：直接写那句错误的话或行为，例如"连续反问孩子：那为什么？那然后呢？"；
- 必须包含：孩子已经答对了，还反复追问"是怎么来的"——低年级孩子凭直觉答题，追问过程只会让他们崩溃；
- 不要写"不要……"这种说教句式，直接写那个行为本身。

请严格输出 JSON，不要输出额外解释，不要输出 markdown 代码块标记。
JSON 字段必须包含：
grade_warning（字符串，超纲时填写说明，不超纲时留空字符串""）,
start_from,
...
async def generate_v2(req: GenerateV2Request):
    ...
    try:
        ...
        result.setdefault("grade_warning", "")
